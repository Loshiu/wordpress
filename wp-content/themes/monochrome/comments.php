...
<div class="comment_form_wrapper" id="respond">
 <?php if (function_exists('wp_login_url')) 
        ...
        { $login_link = bloginfo('wpurl') . '/wp-login.php?redirect_to=' . urlencode(get_permalink()); }
 ?>
...
<p><?php printf(__('Logged in as <a href="%1$s">%2$s</a>.', 'monochrome'), get_bloginfo('wpurl') . '/wp-admin/profile.php', $user_identity); ?><span><a href="<?php echo wp_logout_url(get_permalink()); ?>" title="<?php _e('Log out of this account', 'monochrome'); ?>"><?php _e('[ Log out ]', 'monochrome'); ?></a></span></p>
<?php } else { ?>
<p><?php printf(__('Logged in as <a href="%1$s">%2$s</a>.', 'monochrome'), get_bloginfo('wpurl') . '/wp-admin/profile.php', $user_identity); ?><span><a href="<?php echo bloginfo('wpurl'); ?>/wp-login.php?action=logout" title="<?php _e('Log out of this account', 'monochrome'); ?>"><?php _e('Log out', 'monochrome'); ?></a></span></p>
<?php } ?>
