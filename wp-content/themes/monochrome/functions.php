...
function monochrome_add_theme_options() {
  global $optionsSaved;
    if(isset($_POST['monochrome_save_options'])) {
      ...
      // information
      if (isset($_POST['show_information'])) {
      ...
      if (isset($_POST['use_google_search'])) {
      ...
      // wp_nav_menu
      if (isset($_POST['use_wp_nav_menu'])) {
      ...
      if (isset($_POST['next_preview_post'])) {
      ...
      $options['disable_comment'] = (bool)true;
      } else {
      $options['disable_comment'] = (bool)false;
      }
...
<br />
<p><?php _e('Show border around image in post area.', 'monochrome'); ?></p>
...
<div class="monochrome_box">
<p><?php _e('Disable comment function.', 'monochrome'); ?></p>
<p><input name="disable_comment" type="checkbox" value="checkbox" <?php if($options['disable_comment']) echo "checked='checked'"; ?> /><?php _e('Disable', 'monochrome'); ?></p>
</div>
...
<?php if (function_exists('comment_reply_link')) { 
    ...
    <li class="comment-reply"><?php comment_reply_link(array_merge( $args, array('add_below' => 'comment-content', 'depth' => $depth, 'max_depth' => $args['max_depth'], 'reply_text' => '<span><span>'.__('REPLY','monochrome').'</span></span>'))) ?></li>
<?php   } else { ?>
